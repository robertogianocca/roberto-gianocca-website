...
export default function VimeoPlayer({ spriteSrc }) {
  ...
  useEffect(() => {
    ...
    vimeoPlayer.on("play", () => setPlaying(true));
    vimeoPlayer.on("pause", () => setPlaying(false));
  ...
  const togglePlay = () => {
    const vimeoPlayer = new Player(playerRef.current);
    if (playing) vimeoPlayer.pause();
    else vimeoPlayer.play();
  };
  ...
  const toggleMute = () => {
    ...
    if (volume > 0) {
      ...
      vimeoPlayer.setVolume(previousVolume);
  ...
  const handleVolumeAreaMouseLeave = (e) => {
    // Check if we're moving to the volume slider
    const relatedTarget = e.relatedTarget;
    if (volumeSliderRef.current && volumeSliderRef.current.contains(relatedTarget)) {
      return; // Don't hide if moving to slider
    }

    // Use setTimeout to ensure we're actually leaving the area
    setTimeout(() => {
      // Check if mouse is still within the volume area or slider
      const volumeArea = volumeAreaRef.current;
      const slider = volumeSliderRef.current;

      if (volumeArea && slider && !volumeArea.matches(":hover") && !slider.matches(":hover")) {
  ...
  const handleVolumeSliderMouseLeave = (e) => {
    // Check if we're moving to the volume button/area
    const relatedTarget = e.relatedTarget;
    if (volumeAreaRef.current && volumeAreaRef.current.contains(relatedTarget)) {
      return; // Don't hide if moving to button
    }

    // Use setTimeout to ensure we're actually leaving
    setTimeout(() => {
      const volumeArea = volumeAreaRef.current;
      const slider = volumeSliderRef.current;
  ...
  return (
    <div ref={containerRef} className="max-w-3xl mx-auto p-4 bg-gray-700 relative">
      {/* Play/Pause */}

      <button
        onClick={togglePlay}
        className="absolute aspect-vdeo top-[50%] left-[50%] hover:bg-gray-700 rounded-lg transition-colors"
      >
        {playing ? <span className="w-10 h-10">⏸️</span> : <span className="w-10 h-10">▶️</span>}
      </button>
      <div ref={playerRef} className="vimeo-player w-full aspect-video"></div>
      {/* -------------------- BLOCCO CONTROLLI -------------------- */}
      <div className="flex flex-col justify-between mt-2 bg-gray-700 text-white rounded">
        {/* -------------------- BOTTONI -------------------- */}
